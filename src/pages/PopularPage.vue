<template>
  <v-container fluid>
    <v-row style="margin: 0 100px">
      <v-col cols="10">
        <h2 style="color: #a1b1cb">Popular movies</h2>
      </v-col>
      <v-col cols="2">
        <v-autocomplete
...
export default {
  ...
  setup() {
    const page = ref();
    const { loading, popularMovies, getPopularMovies, totalP } = useMovies();
    const selectedOption = ref("");
    const currentMovies = ref([]);
    const loadMoreMovies = async () => {
      try {
        await getPopularMovies(page.value);
        currentMovies.value = [...popularMovies.value];
      } catch (err) {
        console.log("Error from loadMorePopularMovies: " + err);
      }
    };
